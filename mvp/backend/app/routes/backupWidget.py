...
mcp_sessions: Dict[WebSocket, ClientSession] = {}
...
async def widget_websocket(
    ...
    try:
        ...
        try:
            ...
            mcp_session = await initialize_mcp_session(agent)
            if mcp_session:
                  
                  mcp_sessions[websocket] = mcp_session
        ...
        asyncio.create_task(handle_openai_messages(openai_ws, websocket,mcp_session))
        ...
        # Cleanup
        if websocket in mcp_sessions:
            session = mcp_sessions.pop(websocket)
            try:
...
async def handle_openai_messages(openai_ws: websockets.WebSocketClientProtocol, client_ws: WebSocket,  mcp_session: Optional[ClientSession] = None):
            ...
            if event_type == "conversation.item.input_audio_transcription.completed":
                ...
                if transcript:
                    ...
                    if mcp_session is None:
                     print("[MCP] MCP session not initialized, skipping tool call")
                     continue
                            
                    result = await mcp_session.call_tool(function_name, {
                    "name": name,
                    "email": email,
                ...
                if assistant_text:
                    ...
                    assistant_text = ""
            elif event_type=="response.function_call_arguments.done" and mcp_session:
                call_id = data.get("call_id")
                function_name = data.get("name")
                arguments_str = data.get("arguments", "{}")
                try:
                    arguments = json.loads(arguments_str)
                    print(f"[MCP] Calling tool: {function_name} with {arguments}")
                    #only handle the hubspot request
                    if function_name == "capture_request_hubspot":
                        #ensure email is present
                        email = arguments.get("email")
                        name = arguments.get("name")
                        products = arguments.get("products_requested",[])
                        service = arguments.get("services_requested",[])
                    ...
                    confirmation = result.get("Your contact info is saved we will get back to you to soon")
                    await client_ws.send_json({
                    ...
                    # Request new response with tool result
                    await openai_ws.send(json.dumps({
                        "type": "response.create"
                    }))
                    
                    print(f"[MCP] Tool result sent back to OpenAI")
